...
Expanded README to include project abstract, motivation, data sources, reproducibility instructions, and results analysis.
...
#### Other files/notes:
decade_statistics.csv contains general info about each decade: num of articles, avg text length, etc.

The plots folder contains 2D visualizations of the nearby embedding neighborhood for "utopian" in each of the models.

All the notebook files are my original mockups for the .py scripts. For transparency, my general workflow was to download a small sample of the total dataset to my local machine and then create a notebook that did more or less what I was hoping to do to the entire dataset. From there, I adapted these notebooks into .py scripts using Qwen-3-Coder-30B on Jan AI. I had never dealt with such a high volume of text data before, so it was helpful for me to work through it with this open source model in order to avoid pitfalls in scaling up. I've kept the notebooks in this repo as a reference.
